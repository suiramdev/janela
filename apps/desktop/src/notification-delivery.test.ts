...
/** A logger whose whole history can be searched for a string that must not be in it. */
function recordingLogger(): { readonly log: Logger; readonly records: Recorded[]; text(): string } {
...
interface FakePlugin extends NotificationPlugin {
  ...
  /** Answers `isPermissionGranted`. */
  alreadyGranted: boolean;
  /** Answers `requestPermission`. */
  response: NotificationPermission;
  /** Rejects `removeActive`, the way the macOS desktop backend does. */
  removalFails: boolean;
  /** Rejects `onAction`, the way a backend with no listener command does. */
  listenerFails: boolean;
  /** Held so a test can settle authorization when it chooses. */
  gate?: { readonly promise: Promise<void>; open(): void };
  /** Fires the registered click handler, if one was registered. */
...
function fakePlugin(): FakePlugin {
  ...
  const plugin: FakePlugin = {
    ...
    async onAction(next: (notification: Options) => void): Promise<unknown> {
...
describe("delivery", () => {
  test("the body reaches the plugin and nothing else", async () => {
    ...
    // Nor does the routing payload: `extra` is ids, so a click needs no content.
  ...
  test("a notification carries the ids a click needs to route", async () => {
    ...
    expect(typeof plugin.sent[0]?.id).toBe("number");
  });
  ...
  test("ids kept for withdrawal are bounded", async () => {
    ...
    // Sequential on purpose: each delivery must find authorization already settled,
    // so this exercises the outstanding bound rather than the in-flight one.
...
describe("clicking", () => {
  ...
  test("a notification we did not post routes nowhere, even half-addressed", async () => {
    ...
    // One id and not the other is the interesting shape: routing on it would
    // activate a session and then focus `undefined`.

...
Code versus spec: the shipped constants are `PROTOCOL_VERSION = 13` and
`MINIMUM_SUPPORTED_VERSION = 13`. They moved together up to 6, for the reason
...
`markSession`. `docs/packages/protocol.md` § Version history carries the
numbered history of what each version added, plus what an older peer does when
it meets a newer one. The code wins.

...
import type { ItemRect } from "./use-fluid-hover.ts";
...
function bridgePair(outer: Run, runs: Run[]) {
  ...
  const [up, lo] = inside;
  // `noUncheckedIndexedAccess`: the length check above is the guarantee, but a
  // destructure does not narrow. One read fixed in place, and every `p.up` /
  // `c.lo` below flows from this return type.
  if (up === undefined || lo === undefined) return null;

...
/** A store with no sessions in it; the removal path never reads one back. */
...
/**
 * A target that records what crossed the socket, and answers confirmations.
 *
 * The confirmation is what makes this worth testing at all: every destructive
 * action here has to ask first, and "asks first" is only observable as an
 * ordering between two calls.
 */
 ...
 *
 * A timer, not a chain of `Promise.resolve()`: the removal paths await a reply
 * and then a confirmation, and counting the turns between them would pin the
 * number of `await`s in the implementation rather than its behaviour.
 */
